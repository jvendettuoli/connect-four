...
// Pull out event listeners, loops for building board to seperate functions
...
function makeHtmlBoard() {
	...
	topRow.addEventListener('mouseleave', handleHover); // ?? Better way to handle mouse hover? --Mouseover would be best here
...
function resizeBoard() {
	...
	scalePieces(); // Bug where this is likely getting debounced during resize and so does not fire. Move to CSS.
}
...
function handleClick(evt) {
	...
	// Sets the style of the board depending on the current player. Currently toggles indicator piece color
	toggleGameBoardStyle();
...
function copyBoard(currentBoard) {
	const copy = currentBoard.map((inner) => {
		return inner.slice();
	});
	return copy;
}
